...
## Preventive followup (not urgent)
Replace the special-vdev consumer SSDs with enterprise PLP SSDs (Solidigm/Intel S-series, Samsung PM). Keep the 3-way mirror intact during the swap — **special-vdev loss = whole-pool loss** — and verify a current BigTank backup first.

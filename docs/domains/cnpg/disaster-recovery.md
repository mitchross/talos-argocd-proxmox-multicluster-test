...
```bash
git add clusters/talos/database/cloudnative-pg/<db>/
...
- **K8sGPT** (in `clusters/talos/monitoring/k8sgpt/`) — detects CNPG Cluster anomalies and
...
- **Import the official CNPG Grafana dashboards.** Upstream publishes
  ...
  Drop into `clusters/talos/monitoring/prometheus-stack/` as ConfigMaps with the Grafana
...
`clusters/talos/database/cnpg-barman-plugin/` but no DB uses it yet. Before
...
replacement is the **Barman Cloud Plugin** (`clusters/talos/database/cnpg-barman-plugin/`,

...
assert_contains "$openshift_output" "OLM packages: none required (storage/LB/DB are Helm-installed)"
assert_contains "$openshift_output" "MetalLB: upstream Helm chart + config via Argo"
assert_contains "$openshift_output" "Gateway DNS: use *.vanillax.xyz via openshift-gateway"
